...
| **Three loop counters made unsigned** | **−1.4%** isa_real, −1.9% even, −2.0% degenerate, −1.6% memory | **Portable, and the cheapest change in this file.** Two signed ints compared with `<` cannot be done in one subtract, so the compiler emits a `call pe, __setflag` to repair the flags on overflow -- and one was inside `same_ci`, the loop that compares a mnemonic, on every line of the source. The fault is already written up here for the output cursor and was still in three more loops. **It has no behavioural signature**: identical bytes, identical output, every host check passing with it and without, so the test is on the generated code -- `test/run.sh` counts the repairs left in `assemble_line` and fails above twelve. zap has signed loop bounds throughout its lexer. |
