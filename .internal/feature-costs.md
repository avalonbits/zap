...
| **Two operand flags that were duplicating bits of `mode`** | **−0.3%** isa_real, −0.6% degenerate | **Portable.** `op->indirect` and `op->has_imm` were exactly the INDIRECT and IMM bits of `op->mode`: set in the same statement, cleared in the same statement, never able to disagree. Two bytes of every operand, copied twice a line, to say what the byte beside them said. zap's `operand` has the same pair. The gain is the copy and nothing else -- the frame going 115 to 111 buys nothing, since the `ix` cliff is at 128. |
| **The other three operand flags packed into `mode` as well** | **neutral — reverted** | Recorded so it is not re-tried. `noreg`, `cc` and `fwd2_neg` carry real bits and `mode` has four spare ones, so this takes the operand to 18 bytes and the frame to 106 -- and measures exactly level on three benchmarks and 0.5% worse on isa_memory. The three bytes come off the `ldir` and go straight back on as read-modify-write: `ld (ix - n), 1` becomes `ld a, (ix - n); or 0x20; ld (ix - n), a` wherever a flag is set, twice a line. **Deleting a field that duplicates another is free money; packing a field that carries something is a wash.** Also needs `ri->aempty` and `ri->ccok` to hold the same bit rather than 1, or `ri->aempty & anone` ANDs 1 against 0x10 and every condition-code instruction stops matching. |
| **The data directives, dispatched from the mnemonic lookup's failure** | **+0.3%** isa_real, 0.0% even, +0.6% degenerate, 0.0% memory | **Portable, and the placement is the whole of it.** A directive appears where a mnemonic would and cannot hide behind a colon the way EQU does -- but it needs no test either, because `mnemonic_of` already returns NULL for every one of them and that branch did nothing but set "unknown instruction". An instruction line pays *nothing*: not a test, not a table, not a character, and `assemble_line` came out 28 instructions smaller because a tail call replaced the error block. zap's `enc_instruction` has the same failure branch. The first cut cost 1.2% on benchmarks containing no directives, from two hot-path faults the feature dragged in: `fwd_result` outlined by gaining one cold caller (**fourth** instance), and a `bool` out-parameter taking the line loop's frame from 13 to 16. |
| **ORG, and the origin becoming a field** | **−0.6%** isa_real, −0.3% even, −0.9% degenerate, −0.3% memory | **Portable, and the reason is the transferable part.** Every address was `DZ_ORG + (o - out)` with a compile-time origin, and making it a field so ORG can move it was expected to cost a load on five hot paths. It *gains*, because a 24-bit constant is not free on this chip: there is no add-immediate, so `DZ_ORG + x` compiled to `ld bc, 262144` -- four bytes -- at five sites, against `ld bc, (ix - 92)` at three. One instruction either way, one byte longer, fetched from RAM with wait states on a machine with no instruction cache. **A constant is not free when it does not fit in an instruction**, and zap has wide literals on its own hot paths. The directive's own rule is worth carrying too: the first ORG in a file relocates and every later one pads, even with nothing emitted between them. |
| **INCLUDE, by re-entering the line loop rather than stacking readers** | **+0.3%** isa_real, even and degenerate; +0.8% memory, unexplained | **Portable, and the shape is the point.** A stack of readers in the assembler's state means the line loop asks "is there a parent file" on every refill, to answer something only an INCLUDE can ask. Recursion puts the whole cost in the include: the loop becomes `run_lines(z)`, an include saves the reader it displaces in its own frame, opens the child, calls `run_lines` again and restores -- and **not one line of the loop changed**. Depth capped at 8 because each level costs three frames, one of them `assemble_line`'s 111 bytes. Two things this machine forces and zap will meet too: MOS has few file handles, so the parent is suspended and resumed around the child; and the file name must outlive the line it came from, because resume reopens by name and the child's refills overwrite the buffer it pointed into. **`br_suspend`/`br_resume` had never worked** -- `fread_` was only ever assigned zero, so a resumed reader restarted its file, which here is an infinite re-read rather than an error. |
| **INCBIN** | free | A blob write: open, size, `out_reserve_n`, read to the cursor, close. No reader, no lines, no saved state -- `emit_fill` with a file in place of the 0xFF. |
| **The operand parser's literal fast path, in the data directives** | **−55%** on `DB n`, **−67%** on a four-value list, −27% on `DW`, **−5.7%** isa_real | **Portable, and the lesson is about where a fast path stops.** `emit_data` sent every item through the general evaluator with a `fwd_reset` of six globals in front of it, so `DB 42` measured **889 cycles a byte against `ld a, 42`'s 349** -- two and a half times an instruction that parses the same number *and* does a mnemonic lookup and a row match. The fix is the fast path the operand parser has had all along; what makes it safe is what it refuses, which is anything whose run does not end where the item ends -- `DB 1+2`, and `0b1010` whose decimal loop stops on the `b`. zap's directives will have the same asymmetry the moment it grows them. |
| **`DS` and `ALIGN` at the end of the output are not written** | correctness, found by a benchmark shape | A file of nothing but `DS 4` is 149,800 bytes here and **zero** in the reference: those two reserve space and it is only materialised when something follows. `ORG` is not the same and does pad. Tracked as where the trailing run ends and how long it is, touched only by `emit_fill`, and dropped once after the fixups -- so nothing on the instruction path knows it exists. Found by generating 256 KiB of one directive, which fourteen hand-written cases had not caught because every one of them had something after the directive. |
| **Label names in blocks that never move, and a pointer instead of an offset** | capacity **9,521 -> 14,616+ labels**; **−1.6%** isa_memory, nothing elsewhere | **Portable, and zap has the same array.** The names were one array grown with `realloc`, and a realloc that has to move holds the old block and the new at once -- 110 KB of names needs 228 KB to grow by eight, on a machine with 512 KB and 175 KB of symbol nodes already out. The nodes had been given never-moving blocks for exactly this reason; the names had not, and `nameoff` was an offset *because* the array moved. Give it blocks and a pointer is correct and one add cheaper on the hottest compare in the symbol table -- which is why the speed shows up on isa_memory and nowhere else. |
| **The output buffer has the same fault and cannot be fixed the same way** | **still fails** -- recorded, not solved | `DB` of strings at 256 KiB is "out of memory for the output" at the same line before and after growing by doubling rather than by 32 KB steps, which lowers the transient peak from about 2x the final size to about 1.5x and was not enough. No constant fixes it: the buffer is sized at a quarter of the source from a comment that says *"these instructions average a shade under a fifth of a byte of output per source byte"* -- true when a source could only hold instructions, and `DB "text"` is two thirds -- but `DS 4096` is ten source bytes and four thousand out, and INCBIN is unbounded. **A contiguous output that has to grow cannot work here when the output is large**; the fix is to write it as it is produced and patch by seeking. Largest known limitation. |
| **A sign in the directive fast path** | **−1.3%** isa_real, −1.6% even | **Portable.** The literal fast path began at `digit_ch`, so `DB 1, 2, 3, -1` sent one item in four to the evaluator for a minus the operand parser has always taken. Found by decomposing a line by truncation rather than by guessing: the directive path was still 15.2% of isa_real after `DB 42` had been made cheap, because isa_real's directive lines are not `DB 42`. **A shape file says what a directive costs; it does not say what the directives in a real mix cost.** |
| **The chain compare, re-measured after the names became pointers** | **−1.0%** isa_memory, nothing on isa_real | The four-shape comparison above chose the index form because the pointer walk generated 24 instructions against 16 -- three live pointers would not fit where two and an index did. That was measured when the name had to be computed as `&z->names[sp->nameoff]`; the name is a pointer in the node now, the third pointer is free, and the shape that lost then wins now. **A rejected alternative is only rejected against the code that was there**, and this one had been written down as settled three commits before it became wrong. It also makes the global and local lookups one idiom instead of two that disagreed. |
| **EQU and the data directives share one literal parse** | EQU of literals **−40%**; isa_real unchanged; the DB shapes **+2.4 to +3.5%** | **Portable, and the trade is the point.** `equ_line` sent every value through the general evaluator, and **four EQUs in five are a plain literal** -- 7,778 of 9,772 in the corpus. Sharing the directives' fast path makes those 40% cheaper and costs the DB shapes 2.4-3.5%; two other shapes were tried to recover it and neither did, so it is register allocation in the merged body rather than the `&p` fault that has bitten twice before. Shipped shared because isa_real is unchanged either way and one copy of a parse that must agree with the reference is worth 3% on a synthetic. |
| **`emit_row`'s opcode assembly is free** | **0%** of isa_real | Recorded so nobody optimises it. Decomposing the emitter says reserving room and then choosing prefixes, running the transforms and folding the operands into the opcode together cost nothing measurable; all 8.3% of `emit_row` is writing the bytes and recording the fixups. |
| **The ordinary instruction shape writes its bytes without the six tests** | **−0.6%** isa_real, −0.3% degenerate | **Portable.** Placing an instruction is six conditionals, and one of them -- whether the opcode follows the displacement rather than precedes it -- is true only for `bit n, (ix+d)` and its relatives, yet was computed on every instruction and tested twice. `prefix1` is zero whenever there is no index register, so one load skips the lot. zap's emitter has the same chain. |
| **`acc * 10` in the decimal accumulators** | **cannot be removed** | Recorded so the reasoning is not repeated. All four `__imulu` calls left in `assemble_line` are this, and a 24-bit add being one instruction means ten is five of them. The compiler recognises `v * 8 + v * 2` and re-forms the multiply, because at `-Oz` the call is *smaller*; the empty-asm barrier that would make the value opaque crashes the eZ80 backend (`unable to legalize instruction: G_ANYEXT`). **There is no per-site way to overrule the size-for-speed trade `-Oz` is making.** |
| **Three loop counters made unsigned** | **−1.4%** isa_real, −1.9% even, −2.0% degenerate, −1.6% memory | **Portable, and the reason first given for it was wrong.** The gain is real and reproduced; the explanation -- that it removes a `call pe, __setflag` -- is not, because that call is *conditional* and a length compared against 3 never overflows. What it buys is a loop body the compiler does better with. **It has no behavioural signature**: identical bytes, identical output, every host check passing either way, so the test is on the generated code as a change detector. zap has signed loop bounds throughout its lexer, and they are worth trying one at a time and timing -- not counting. |
| **The other eight `__setflag` calls, removed** | **+0.3% — reverted** | Recorded so the disassembly is not mined again. Comparing token lengths as unsigned takes `assemble_line` from twelve flag repairs to four and the program from 9,456 instructions to 9,403, and measures one to two cycles a byte **slower** on all four benchmarks. `call pe, __setflag` is a conditional call taken only on overflow, so each costs a fetch and an untaken branch rather than a call. **The count is not the cost, and a static count of anything is not a measurement.** |
| **ASSUME ADL** | **+0.9%** isa_real, +0.8% even | **Portable, and the biggest unblocker there is**: `.assume adl=1` is line 1 of every file in the reference's Opcodes category, so one directive took the corpus from 58 of 131 to **68**. The mode was a compile-time `true` and folding `(adl ? 3 : 2)` to a constant is what it cost to give it up. Note against the ORG row above: that constant was 0x040000, which does not fit an instruction, so making it a field was *cheaper*; this one is `3`, which does, so it is dearer. **Whether a constant is free depends on the constant.** Storing the width instead of the mode, so the select disappears, measured 2.6% worse again and is recorded as rejected. |
| **Conditional assembly** | **free** -- 352/359/339/371 unchanged | **Portable.** `IF`/`ELSE`/`ENDIF`, and **nesting is not supported by the reference**, which makes it one flag rather than a stack. The test goes after the token is scanned and before the label is defined -- a label in a switched-off branch must not be defined -- and costs nothing measurable. Corpus 68 of 131 to **74**. Two things worth carrying: `ELSE` toggles and toggles again; and **the reference does not compare** -- `IF a == b` evaluates `a` and throws the rest away, so `IF 0 == 0` is false there and `IF 1 == 2` is true. That decides bytes, so `-ez80` reproduces it and the default means what it says, exactly as with precedence. |
| **Macros** | **+1.1%** isa_real, +1.1% even | **Portable, and the expansion mechanism is the transferable part.** A macro expansion is an include from memory: build the substituted text, set the parent reader aside, open a reader over it, re-enter the line loop. `br_open_mem` was written for this and had been waiting. Two things that will catch zap too: **the line loop could not read a memory reader at all** -- it starts with an empty window and `br_fill_lines` returns false for memory, which reads as end of file, so every expansion assembled to nothing quietly; and **`br_suspend` refuses a memory reader**, so a macro invoking another must suspend only a file. Substitution is textual and by whole identifier -- measured: `db 10-x` with `x` bound to `1+1` is ten, not eight. Corpus 74 of 131 to **81**. |
| **Splitting `assemble_line`** | **+7.6 to +12.3% -- rejected** | Recorded so it is not re-attempted, and it had been on the list as an assumed win. The function is 3,858 instructions, 51% of the program, with a 115-byte frame; outlining `parse_operand` takes it to 1,454 with an 89-byte frame and the whole program 12% smaller, and costs **7.6% on isa_real and 12.3% on isa_degenerate**. Two reasons, both already written down here in smaller print: **the frame was never over the cliff** -- `ix` displacement is a signed byte, the cliff is 128 and the frame was 115, so the 26 bytes bought nothing -- and inlined, `parse_operand` writes the operand at constant `ix` offsets, while called it writes 23 bytes through a pointer. Third and largest instance of *a fixed frame slot is cheaper than a pointer to anything*. **The lever is making a `dop` smaller, not making `assemble_line` shorter.** |
| **EQU** | **+0.6%** isa_real, +0.6% even, +0.9% degenerate, +0.3% memory | **Portable, and the placement is the transferable part.** Fifteen lines of feature and four measurements to decide where they go, spanning 1.8%. Keeping the defined symbol in a `sym*` local cost three bytes of frame and `assemble_line` lost `iy` as its line pointer; taking the address of that pointer cost the same slot again; calling `sym_define` from the new code de-inlined it from the label path and cost 2.9% on isa_memory alone. The shipped version tests for `EQU` *after* the early return that ends a bare `label:` line, so a label on its own line never reaches it, and re-finds the symbol by name rather than carrying it. **A feature that runs on 0.1% of lines must be reached from a place the other 99.9% do not pass through.** |
