> **One assembler.** Everything below was measured on `src/zap.c`. The early
> entries were written while it was being built beside an older assembler it
> has since replaced; where the two are compared, the older one is called
> exactly that.

# Where zap stands, and what is left

Notes to self, started 2026-09-05. Everything here is measured, not estimated,
unless it says otherwise. Timings are on fab-agon-emulator 1.2.4 without `-u`, which
decouples the guest clock from guest work and makes every number meaningless.

**Two assemblers appear below and their numbers are not comparable.** Nearly
everything is zap; the `Where the earlier assembler stood` section is the
program it replaced, kept because the memory findings in it still apply.

## zap against the reference, on both machines (2026-09-06)
...
| source | zap | ez80asm | ez80asm `-m` | zap is | zap |
|---|---|---|---|---|---|
...
more: a `.assume adl=1` and an `.org 0x040000` that zap does not need because
...
table out of the picture. zap's margin there is 6.92x against 7.27x on
...
## Where the earlier assembler stood (2026-09-05)
...
isa_real, −0.7% on isa_even, both above the noise. **Not taken.** zap is meant
...
`opcodes.s` was regenerated by "filtering the corpus through zap and keeping
what it accepts", which made it a record of what zap already got right. A form
zap assembled wrongly was removed by the very filter meant to catch it.
...
quietly excluding the forms zap was worst at. **Every timing before that
...
o777, q777, h42, @777, &42 all refused. zap already agreed on all 36 probes;
...
Added 2026-09-06. zap now has label definitions, references, forward
...
percent against a factor of three is not a close decision, and zap now uses
...
fit**: zap runs out of memory at line 28,673 of 30,920, 93% of the way
...
* **A generated label can collide with the literal syntax.** The first names
  ...
  **zap accepted `a00h:` where ez80asm rejects it.** Fixed: a definition whose
  ...
  `123:`, and accepts `beef:`, `zzh:`, `h:` and `a0h_x:`; zap now agrees on
  all eighteen probes.
...
## zap against the reference on the same file
...
* **zap** stops in seconds: `out of memory for labels` at line 28,673 of
...
So zap's ceiling is not unusually low -- it is the one that says so quickly and
in words. Failing loudly at 93% is worth more than not finishing, and that is a
property to keep rather than an accident.

## Where zap's time goes, priced rather than guessed (2026-09-06)
...
**Data-only duplication.** Write one of zap's tables with every entry
...
have each cost more than they saved (see zap-to-zap.md rows 59-62). Row
...
message confirms -- and zap reaches the same answers from a separate table
...
anonymous one -- and zap does the same.
...
zap.c, every one checked to leave the output identical. Taken on the 4.94s
...
which is what the reference does. It is the first place zap knowingly computes
...
to escape another's. zap does what every language does, and a ZDS program that
...
zap binds unary tightest, as C, ez80asm and every other assembler do, giving -1
...
unaries (`~~255`); ez80asm takes one and errors on a second, and so does zap.
...
The obstacle is that parentheses already mean indirection, and zap decides that
...
### zap accepts more than ZDS, and agrees wherever ZDS accepts
...
is not on the table. So zap accepts a superset, and on the intersection the two
agree byte for byte.
...
levels, assembled by ZDS and by zap and compared byte for byte: **500 agree, 0
...
`sym_intern` as 8.1% -- and both are noted in `zap-to-zap.md` as needing to be
...
All four sources still assemble byte-identically between zap and the reference,
...
same finding as two entries already in `zap-to-zap.md` -- the operand passed as
...
* **Every file ends on a scope boundary.** The reference scopes locals *per
  ...
  there, in either direction, and zap is more permissive. A file cut at an
...
The reference scopes local labels per file and zap does not: a local defined in
...
All five sources still assemble byte-identically between zap and the reference.
...
that zap has an input size it fails on, and the failure is silent.
...
## The size zap fails at, and half of it fixed (2026-09-07)
...
zap detects it and names the line. What was silent was the harness: my scratch
...
`zap-to-zap.md` records four shapes of the chain compare measured against each
...
So the test is on the generated code: `test/run.sh` now builds `zap.c` for the
...
is one pass rather than two. The pattern `x[-1]` appears nowhere else in zap.
