...
[`main()`](../src/zap.c#L1795) ·
[`parse_args()`](../src/zap.c#L923) ·
[`run()`](../src/zap.c#L610) ·
[`run_lines()`](../src/zap.c#L468) ·
[`scope_end()`](../src/symtab.c#L852) ·
[`resolve_fixups()`](../src/zap.c#L444) ·
[`out_flush()`](../src/directive.c#L59) ·
[`resolve_late()`](../src/zap.c#L402)
...
has run out, [`fix_sweep()`](../src/symtab.c#L1073) settles every entry whose
...
| | resolved by | holds |
|---|---|---|
| fixup | [`patch_fixup()`](../src/symtab.c#L638) | one or two symbols, an addend, a width, an offset |
| deferred expression | [`resolve_deferred()`](../src/zap.c#L262) | expression text a fixup can't represent |
| deferred fill | [`resolve_fills()`](../src/zap.c#L312) | a `BLK` whose fill value wasn't known yet |

A [`fixup`](../src/zap.h#L709)'s width is usually a byte count (1 to 4) or 0 for
...
- Three "folds", where the value goes into the opcode byte itself: a bit
  ...
  works when `n` is defined later. [`fold_mask()`](../src/symtab.c#L593) turns
...
finished bytes in a [`latepatch`](../src/zap.h#L631). It stores bytes rather
...
At the end, [`resolve_late()`](../src/zap.c#L402) makes one ascending pass over
...
and then drops back. A [`laterun`](../src/zap.h#L654) marks one of those
...
[`state`](../src/symtab.c#L235), of type [`zap_state`](../src/zap.h#L1213),
...
[`include_file()`](../src/directive.c#L874) opens a second reader and runs the
...
[`equ_line()`](../src/expr.c#L704) ·
[`mnemonic_of()`](../src/insn.h#L87) ·
[`parse_operand()`](../src/expr.h#L109) ·
[`match_row()`](../src/insn.h#L150) ·
[`emit_row()`](../src/insn.h#L327) ·
[`directive_line()`](../src/directive.c#L1076) ·
...
[`sym_intern()`](../src/symtab.c#L465)) are added to the table the first time
...
Local labels (`@name`, [`loc_intern()`](../src/symtab.c#L915)) belong to the
...
in [`scope_end()`](../src/symtab.c#L852) makes every bucket read as empty
...
Each fixup is 16 bytes, and [`fix_add()`](../src/symtab.c#L1110) grows the list
...
When memory runs out, [`fix_sweep()`](../src/symtab.c#L1073) settles what it
...
A fixup can be settled early only if ([`fix_ready()`](../src/symtab.c#L1043)):
...
[`fold_subs()`](../src/symtab.c#L808) uses them at every scope end, so
...
([`expr_value()`](../src/expr.c#L642), [`expr_atom()`](../src/expr.c#L282))
...
[`directive_line()`](../src/directive.c#L1076) handles them.
...
- `DS` reserves ([`fill_take()`](../src/directive.c#L637)) while `BLK` emits
  ([`emit_block()`](../src/directive.c#L684)). A reservation is just a count:
...
- `ORG` padding isn't a reservation. It goes through
  [`fill_put()`](../src/directive.c#L594) and is written immediately, as
...
[`zap_err`](../src/zap.h#L502), and the messages live in
...
[`err_line()`](../src/symtab.c#L255) ·
[`err_tok()`](../src/symtab.h#L52) ·
[`report()`](../src/zap.c#L1738)
...
The truncation warning, [`warn_trunc()`](../src/zap.c#L1662), is different
...
[`list_line()`](../src/zap.c#L1204) and [`list_out()`](../src/zap.c#L1182). A
...
patched. [`lstfix_add()`](../src/zap.c#L1310) remembers those lines, and
[`lstfix_apply()`](../src/zap.c#L1343) rewrites their byte columns from the
...
[`write_symbols()`](../src/zap.c#L1522) writes the sorted global symbols in
ez80asm's format, and [`write_stats()`](../src/zap.c#L1588) prints resource
...
- A directive needs a `DIR_` constant, a spelling in
  ...
  [`directive_line()`](../src/directive.c#L1076), a test file under
  `test/cases` compared against ez80asm, and a row in the README's directive
  table.
- A diagnostic needs a [`zap_err`](../src/zap.h#L502) code and one line in the
