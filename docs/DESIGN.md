...
has run out, [`fix_sweep()`](../src/symtab.c#L1074) settles every entry whose
...
A [`fixup`](../src/zap.h#L710)'s width is usually a byte count (1 to 4) or 0 for
...
finished bytes in a [`latepatch`](../src/zap.h#L632). It stores bytes rather
...
and then drops back. A [`laterun`](../src/zap.h#L655) marks one of those
...
[`state`](../src/symtab.c#L236), of type [`zap_state`](../src/zap.h#L1214),
...
Local labels (`@name`, [`loc_intern()`](../src/symtab.c#L916)) belong to the
...
in [`scope_end()`](../src/symtab.c#L853) makes every bucket read as empty
...
Each fixup is 16 bytes, and [`fix_add()`](../src/symtab.c#L1111) grows the list
...
When memory runs out, [`fix_sweep()`](../src/symtab.c#L1074) settles what it
...
A fixup can be settled early only if ([`fix_ready()`](../src/symtab.c#L1044)):
...
[`fold_subs()`](../src/symtab.c#L809) uses them at every scope end, so
