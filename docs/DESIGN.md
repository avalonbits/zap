...
has run out, [`fix_sweep()`](../src/symtab.c#L1100) settles every entry whose
...
| | resolved by | holds |
|---|---|---|
| fixup | [`patch_fixup()`](../src/symtab.c#L665) | one or two symbols, an addend, a width, an offset |
...
A [`fixup`](../src/zap.h#L711)'s width is usually a byte count (1 to 4) or 0 for
...
- Three "folds", where the value goes into the opcode byte itself: a bit
  ...
  works when `n` is defined later. [`fold_mask()`](../src/symtab.c#L620) turns
...
finished bytes in a [`latepatch`](../src/zap.h#L633). It stores bytes rather
...
and then drops back. A [`laterun`](../src/zap.h#L656) marks one of those
...
[`state`](../src/symtab.c#L236), of type [`zap_state`](../src/zap.h#L1219),
...
[`sym_intern()`](../src/symtab.c#L492)) are added to the table the first time
...
Local labels (`@name`, [`loc_intern()`](../src/symtab.c#L942)) belong to the
...
in [`scope_end()`](../src/symtab.c#L879) makes every bucket read as empty
...
Each fixup is 16 bytes, and [`fix_add()`](../src/symtab.c#L1137) grows the list
...
When memory runs out, [`fix_sweep()`](../src/symtab.c#L1100) settles what it
...
A fixup can be settled early only if ([`fix_ready()`](../src/symtab.c#L1070)):
...
[`fold_subs()`](../src/symtab.c#L835) uses them at every scope end, so
...
[`macmark`](../src/zap.h#L345): an offset into the body, a parameter number and
a length.
...
[`zap_err`](../src/zap.h#L504), and the messages live in
...
[`err_line()`](../src/symtab.c#L282) ·
[`err_tok()`](../src/symtab.c#L262) ·
...
- A directive needs a `DIR_` constant, a spelling in
  ...
  table.
- A diagnostic needs a [`zap_err`](../src/zap.h#L504) code and one line in the
