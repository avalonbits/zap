...
Six sources, fixed, so that a number taken today can be compared with one taken
in six months. Run them with `test/bench/bench.sh`.
...
| `isa_even` | 256 KB generated by `gen_isa.sh even` | Every form zap assembles, the same number of times, so nothing is over-weighted |
| `isa_real` | 256 KB generated by `gen_isa.sh real` | Every form again, weighted by how often each appears in BBC BASIC and Rokky |
| `isa_include` | a tree of ten sources and three blobs, `gen_isa.sh include` | A reader suspended, resumed and carried on with, ten times over a tree four deep. Not comparable with the rows above: opening thirteen files on an emulated card is real work the single-file sources never do |

The first two live in `test/corpus`, vendored with the reference's test suite.
The rest are generated deterministically -- the line at index *i* is a pure
...
it is met**: 0.17x on bbcbasic, 0.21x on rokky, 0.16x on synth, and 0.16x to
0.17x on the three isa sources.
