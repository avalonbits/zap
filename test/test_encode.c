/*
 * Host tests for zap's instruction encoding.
 *
 * Expected bytes come from the vendored ez80asm (test/ref), generated rather
 * than transcribed. zap exists to be measured against that assembler, so a
 * byte it disagrees on is a bug in zap whatever the manual says.
 *
...
int main(void) {
     ...
     * must zap, rather than emitting a wrapped byte. */
     ...
     * zap reads a line once and never returns to it, so the bytes go down as
     ...
     * this pass for the wrong reason, zap having no directives. */
    {
     ...
     * and `a0h_x:`, none of which are numbers. zap accepted every one of
     ...
     * makes a reference from outside the scope find nothing. zap reaches the
     ...
     *
     * zap gives operators the precedence a reader expects. The reference
     ...
     * these -- and zap follows neither assembler's quirk. A ZDS program that
